...
IMAGENETTE_DIR_PATH = '/scratch/scratch6/gopi/gopi/Imagenette_sample'

TRAIN_PERTURBATIONS_DIR = '/scratch/scratch6/gopi/gopi/CreatingDataset/ImagenetteDataset_sample/train/PerturbationsFGSM'
TRAIN_ADVERSARIAL_DIR = '/scratch/scratch6/gopi/gopi/CreatingDataset/ImagenetteDataset_sample/train/AdversarialFGSM'

VAL_PERTURBATIONS_DIR = '/scratch/scratch6/gopi/gopi/CreatingDataset/ImagenetteDataset_sample/val/PerturbationsFGSM'
VAL_ADVERSARIAL_DIR = '/scratch/scratch6/gopi/gopi/CreatingDataset/ImagenetteDataset_sample/val/AdversarialFGSM'
...
def create_adversarial_pattern_fgsm(input_image, input_label,my_model):
  perturbations = np.zeros_like(input_image)
  adv_image = input_image
  #prev_probs=[]
  with tf.GradientTape() as tape:
  ...
  #predict_label_my_model(adv_image)
...
if __name__ == '__main__':
    ...
    for idx, each_input_image in enumerate(all_files_list):
        ...
        perturbations = create_adversarial_pattern_fgsm(tf.convert_to_tensor(image, dtype=tf.float32),label, my_model)
        ...
        for eps in EPS_LIST:
            ...
            result_perturbations_dir = VAL_PERTURBATIONS_DIR + f"/for_{eps}"
            save_image(perturbations, f"perturbations_{tail}", result_perturbations_dir)

            result_adversarial_dir = VAL_ADVERSARIAL_DIR + f"/for_{eps}"
